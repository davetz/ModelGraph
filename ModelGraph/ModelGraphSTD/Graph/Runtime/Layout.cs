...
namespace ModelGraphSTD
{
    ...
    {
        internal static (int count, int[] nquad, int[] nsect, int[] sectEdge, (Edge edge, Node other, EdgeRotator conn, (int x, int y) bend, double slope, int ord1, int ord2, byte quad, byte sect, byte tuple)[]) 
        ...
        */
            ...
            var E = new (Edge edge, Node other, EdgeRotator conn, (int x, int y) bend, double slope, int ord1, int ord2, byte quad, byte sect, byte tuple)[count];
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                E[i].ord1 = n1.Graph.Nodes.IndexOf(other);
                E[i].other = other;
                E[i].bend = bend;
            ...
            {
                ext.Point2 = E[i].bend;
                var (quad, sect, slope) = XYPair.QuadSectSlope(ext.Delta);
            ...
            void AssignTupleQuadSect()
            {
                if (tupleT != 0 && countT > 1)
                {
                    ...
                    var (quad, sect, slope) = XYPair.QuadSectSlope(ex.Delta);
