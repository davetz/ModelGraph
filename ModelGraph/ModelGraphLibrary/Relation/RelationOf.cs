 ...
 */
    public class RelationOf<T1, T2> : Relation where T1 : Item where T2 : Item
    {
        ...
        override internal void InsertLink(Item parentItem, Item childItem, IndexPair index)
        {
            var parent = parentItem as T1;
            var child = childItem as T2;
            if (parent == null || child == null) return;

            InsertLink(parent, child, index);
        }

        internal void InsertLink(T1 parent, T2 child, IndexPair index)
        {
            switch (Pairing)
            {
                case Pairing.OneToOne:
                    ...
                    _parents1.InsertLink(child, parent, index.Parent);
                    _children1.InsertLink(parent, child, index.Child);
                    break;
                ...
                case Pairing.OneToMany:
                    ...
                    _parents1.InsertLink(child, parent, index.Parent);
                    _children2.InsertLink(parent, child, index.Child);
                    break;
                ...
                case Pairing.ManyToMany:
                    ...
                    _parents2.InsertLink(child, parent, index.Parent);
                    _children2.InsertLink(parent, child, index.Child);
                    break;
            }
        }
        override internal IndexPair AppendLink(Item parentItem, Item childItem)
        {
            var parent = parentItem as T1;
            var child = childItem as T2;
            if (parent == null || child == null) return new IndexPair(-1, -1);

            return AppendLink(parent, child);
        }
        internal IndexPair AppendLink(T1 parent, T2 child)
        {
            switch (Pairing)
            {
                case Pairing.OneToOne:
        ...
        override internal IndexPair GetIndex(Item parentItem, Item childItem)
        {
            var index = new IndexPair(-1, -1);

            var parent = parentItem as T1;
            var child = childItem as T2;
            return (parent == null || child == null) ? new IndexPair(-1, -1) : GetIndex(parent, child);
        }
        internal IndexPair GetIndex(T1 parent, T2 child)
        {
            var index = new IndexPair(-1, -1);

            if (_parents1 != null) index.Parent = _parents1.GetIndex(child, parent);
            else if (_parents2 != null) index.Parent = _parents2.GetIndex(child, parent);

            if (_children2 != null) index.Child = _children2.GetIndex(parent, child);
            else if (_children1 != null) index.Child = _children1.GetIndex(parent, child);
