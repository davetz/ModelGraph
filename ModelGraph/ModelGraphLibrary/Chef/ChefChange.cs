    ...
    public partial class Chef
    {
        ...
        internal void Undo(ItemCreated chng)
        {
            ...
            if (item.IsRowX)
            {
                ...
                var cols = TableX_ColumnX.GetChildren(tbl);
                if (cols != null) { foreach (var col in cols) { col.RemoveValue(row); } }
            }
            else
            {
                var store = item.Owner as Store;
        ...
        internal void Redo(ItemCreated chng)
        {
            ...
            if (item.IsRowX)
            {
                ...
                if (chng.Columns != null)
                {
                    ...
                    for (int i = 0; i < N; i++)
                    {
                        chng.Columns[i].TrySetValue(row, chng.Values[i]);
                    }
                }
            }
            else
        ...
        private void SetValue(ItemModel model, string newValue)
        {
            ...
            var oldValue = prop.GetValue(itm1);
            ...
            if (prop.TrySetValue(itm1, newValue))
            {
        ...
        {
            if (chng.Item.IsValid && chng.CanUndo && chng.Property.TrySetValue(chng.Item, chng.OldValue))
            {
                MinorDelta += 1;
        ...
        {
            if (chng.Item.IsValid && chng.CanRedo && chng.Property.TrySetValue(chng.Item, chng.NewValue))
            {
                MinorDelta += 1;
        ...
        internal void Redo(ItemRemoved cg)
        {
            ...
            for (int i = 0; i < N; i++) { cg.Columns[i].RemoveValue(itm); }
        ...
        internal void Undo(ItemRemoved cg)
        {
            ...
            var N = (cg.Columns != null) ? cg.Columns.Length : 0;
            for (int i = 0; i < N; i++) { cg.Columns[i].TrySetValue(itm, cg.Values[i]); }
        ...
        internal void ItemLinked(Relation relation, Item item1, Item item2)
        {
            ...
            var index = relation.AppendLink(item1, item2);
            var chg = new ItemLinked(_changeSet, relation, item1, item2, index, name);
        }
        internal void Undo(ItemLinked chng)
        {
            MajorDelta += 1;

            chng.Relation.RemoveLink(chng.Item1, chng.Item2);
        }
        ...
        internal void Redo(ItemLinked chng)
        {
            ...
            chng.Relation.AppendLink(chng.Item1, chng.Item2);
        }
        ...
        internal void MarkItemUnlinked(Relation rel, Item item1, Item item2)
        {
            ...
            var index = rel.GetIndex(item1, item2);
            ...
            var name = $" [{rnam}]   ({nam1}) --> ({nam2})";
            var chg = new ItemUnLinked(_changeSet, rel, item1, item2, index, name);
        }
        ...
        internal void Undo(ItemUnLinked cg)
        {
            ...
            cg.Relation.InsertLink(cg.Parent, cg.Child, cg.Index);
            cg.IsUndone = true;
        }
        #endregion
