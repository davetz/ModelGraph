 ...
 */
    public partial class Chef
    {
        ...
        private class ItemModelStack
        {
            ...
            internal ItemModelStack(int capacity = minLength)
            {
            ...
            #region PopNext  ==================================================
            /// <summary>
            /// Get the next unvisited model in the ItemModelTree
            /// </summary>
            /// <returns></returns>
        ...
        public bool ValidateModelTree(RootModel root, ChangeType change = ChangeType.NoChange)
        {
            ...
            var modelStack = new ItemModelStack();
        ...
        private ItemModel NewPropertyModel(ItemModel model, byte level, Item item, Property prop)
        {
            ...
            else if (prop.ValueType == ValueType.Bool)
                ...
                if (N > 0)
                {
                    ...
                    for (int i = 0; i < N; i++)
                    {
                        ...
                        if (!TryGetOldModel(model, Trait.MockChef_M, oldModels, i, itm))
                            model.ChildModels[i] = new ItemModel(model, Trait.MockChef_M, 0, itm, null, null, MockChef_M);
                    }
                }
            ...
            if (root != null)  // get the data for this root.ModelAction
            {
                switch (root.ModelAction)
                {
                    ...
                    case ModelAction.ModelRefresh:
                        ...
                        root.ModelValue = col.GetValue(itm);
            ...
            if (root != null)  // get the data for this root.ModelAction
            {
                switch (root.ModelAction)
                {
                    ...
                    case ModelAction.ModelRefresh:
                        ...
                        root.ModelName = col.Name;
                        root.ModelIsChecked = col.GetBool(itm);
                        break;
            ...
            if (root != null)  // get the data for this root.ModelAction
            {
                ...
                switch (root.ModelAction)
                {
                    ...
                    case ModelAction.ModelRefresh:
                        ...
                        root.ModelValue = pro.GetValue(itm);
                        if (root.ModelValue == null) root.ModelValue = string.Empty;
                        break;
            ...
            if (root != null)  // get the data for this root.ModelAction
            {
                ...
                switch (root.ModelAction)
                {
                    ...
                    case ModelAction.ModelRefresh:
                        ...
                        root.ModelName = _resourceLoader.GetString(pro.NameKey);
                        root.ModelIsChecked = pro.GetBool(itm);
                        break;
            ...
            if (root != null)  // get the data for this root.ModelAction
            {
                ...
                switch (root.ModelAction)
                {
                    ...
                    case ModelAction.ModelRefresh:
                        ...
                        root.ModelValue = cd.GetValue(item);
                        if (root.ModelValue == null) root.ModelValue = string.Empty;
                        break;
        ...
        internal void ViewView_M(ItemModel model, RootModel root)
        {
            ...
            else  // validate the list of child models
            {
                ...
                if (model.IsExpandedLeft)
                {
                    ...
                    //            if (!TryGetOldModel(model, Trait.ViewXProperty_M, oldModels, i, px))
                    //                model.ChildModels[i] = new ItemModel(model, Trait.ViewXProperty_M, level, px, ViewX_P, view, ViewXProperty_M);
                    //        }
                    //    }
                    //    if (L2 > 0)
                    //    {
                    ...
                    //            var qx = queryList[j];
                    //            if (!TryGetOldModel(model, Trait.ViewXQuery_M, oldModels, i, qx))
                    //                model.ChildModels[i] = new ItemModel(model, Trait.ViewXQuery_M, level, qx, ViewX_QueryX, view, ViewQuery_M);
                    //        }
                    //    }
                    //    if (L3 > 0)
                    //    {
                    ...
                    //            var vx = viewList[j];
                    //            if (!TryGetOldModel(model, Trait.ViewXView_M, oldModels, i, vx))
                    //                model.ChildModels[i] = new ItemModel(model, Trait.ViewXView_M, level, vx, ViewX_ViewX, view, ViewView_M);
                    //        }
                    //    }
                    //}
                }
        ...
        internal void ColumnX_X(ItemModel model, RootModel root)
        {
            ...
            else  // validate the list of child models
            {
                if (model.IsExpandedRight)
                {
                    var sp = new Property[] { _columnXNameProperty, _columnXSummaryProperty, _columnXTypeOfProperty, _columnXIsChoiceProperty, _columnXDefaultProperty };
                    var N = sp.Length;
                    ...
                    if (model.IsExpandedRight)
                    {
                        switch (cx.CompuType)
                        {
                            case CompuType.RowValue:
                                sp = new Property[] { _computeXNameProperty, _computeXSummaryProperty, _computeXCompuTypeProperty, _computeXSelectProperty, _computeXNativeTypeProperty };
                                break;
                            case CompuType.RelatedValue:
                                sp = new Property[] { _computeXNameProperty, _computeXSummaryProperty, _computeXCompuTypeProperty, _computeXNativeTypeProperty };
                                break;
                            case CompuType.NumericValueSet:
                                sp = new Property[] { _computeXNameProperty, _computeXSummaryProperty, _computeXCompuTypeProperty, _computeXNumericSetProperty, _computeXNativeTypeProperty };
                                break;
                            case CompuType.CompositeString:
                            case CompuType.CompositeReversed:
                                sp = new Property[] { _computeXNameProperty, _computeXSummaryProperty, _computeXCompuTypeProperty, _computeXSeparatorProperty, _computeXSelectProperty, _computeXNativeTypeProperty };
                                break;
                        }
                        R = sp.Length;
                    }
                ...
                if (N > 0)
                {
                    ...
                    for (int i = 0; i < N; i++)
                    {
                        ...
                        if (!TryGetOldModel(model, Trait.ComputeX_M, oldModels, i, itm))
                            model.ChildModels[i] = new ItemModel(model, Trait.ComputeX_M, level, itm, null, null, ComputeX_M);
                    }
                }
                else
                {
        ...
        internal void ValueXHead_X(ItemModel model, RootModel root)
        {
            ...
            {
                var sp = new Property[] { _queryXRelationProperty, _queryXIsReversedProperty, _queryXRootWhereProperty, _queryXSelectProperty, _queryXNativeTypeProperty };
                var R = model.IsExpandedRight ? sp.Length : 0;
