...
{
    internal class EvaluateUIntAnd1 : EvaluateBase
    {
        internal EvaluateUIntAnd1(ComputeStep step) : base(step) { }

        uint GetVal(ComputeStep step)
        {
            var N = step.Count;
            step.Inputs[0].GetValue(out uint val);
            for (int i = 1; i < N; i++)
            {
                step.Inputs[i].GetValue(out byte v1);
                val &= v1;
            }
