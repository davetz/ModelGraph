...
{
    internal class EvaluateConcat : EvaluateBase
    {
        internal EvaluateConcat(ComputeStep step) : base(step) { }

        string GetVal(ComputeStep step)
        {
            var N = step.Count;
            step.Inputs[0].GetValue(out string val);
            for (int i = 1; i < N; i++)
            {
                step.Inputs[i].GetValue(out string v1);
                val += v1;
            }
            return val;
        }
