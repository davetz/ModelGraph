...
{
    internal class EvaluateIntegerMinus : EvaluateBase
    {
        internal EvaluateIntegerMinus(ComputeStep step) : base(step) { }

        int GetVal(ComputeStep step)
        {
            var N = step.Count;
            step.Inputs[0].GetValue(out int val);
            for (int i = 1; i < N; i++)
            {
                step.Inputs[i].GetValue(out int v1);
                val -= v1;
            }
